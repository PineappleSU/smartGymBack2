...
public class ArchivesLogAspect {
	...
	public void doBeforeInServiceLayer(JoinPoint joinPoint) throws Exception {
		...
		if (inputParamMap.containsKey("user_wxid"))
			userWxId = ((String[]) inputParamMap.get("user_wxid"))[0];

		if (!StringUtils.isBlank(userWxId)) {
			AppUser appUser = new AppUser();
			appUser.setWxId(userWxId);
			SGResult result = appUserService.getAppUserByDetail(appUser);
			if (result.isOK()) {
				AppUser reUser = (AppUser) result.getData();
				if (reUser != null) {
					userName = reUser.getUserName();
