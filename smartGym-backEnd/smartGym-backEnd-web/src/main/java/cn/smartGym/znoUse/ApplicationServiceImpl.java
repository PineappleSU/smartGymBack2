...
public class ApplicationServiceImpl implements ApplicationService {
	...
	public SGResult updateUser(AppUser userUpdate) {
		...
		Application applicationExample = new Application();
		applicationExample.setName(userUpdate.getUserName());
	 ...
	 */
	@Override
	public List<Application> getApplicationListByItemIdsAndCollegeAndStatus(List<Long> itemIdsIds, String college,
			Integer... statuses) {
		ApplicationExample example = new ApplicationExample();
		Criteria criteria = example.createCriteria();
		if (itemIdsIds != null && itemIdsIds.size() !=0)
			criteria.andIdIn(itemIdsIds);
...
//		Item item = new Item();
//		item.setGame(game);
//		List<Event> events = eventService.getEventByDetailsAndStatuses(item, 1,2,3);
//		List<String> eventIds = eventService.getEventIdsByEvents(events);
//		
//		List<Application> applications = getApplicationListByItemIdsAndCollegeAndStatus(eventIds, college, 2, 3);
